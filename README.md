...
The bot now starts a Windows input watcher before automation begins. If a real user presses any keyboard key, mouse button, or mouse wheel, the bot stops immediately. Synthetic input generated by the bot is ignored, so the bot should not stop itself. Moving the mouse without pressing anything does not stop the bot.
